...
def sendEmail(rec, company_name, invoice_no, invoice_amt, invoice_date,  status, body = None):
    ...
    if body is None:
        body = '''The invoice to {} dated {} of amount {} has a change of status.
        It's status is updated to '{}'.
        
        Please Do not respond to this email. It is autogenerated.
        '''.format(company_name, invoice_date, invoice_amt, statusText)
        
    subject = "{} Invoice status update".format(company_name)
...
def approverApprovedAction():
    ...
    print("invoice_id: ", row_id, file = sys.stderr)

    #get invoice details
    cur.execute('''select company_contact, company_name from company where company_id = (select invoice_vendor from invoice where invoice_id = ?)''', (row_id,))
    company_contact, company_name = cur.fetchone()
...
def vendorAddInvoiceAction():
    ...
    if file and allowed_file(file.filename):
        ...
        # cursor.execute('''select company_contact, company_name from company where company_id = (select invoice_vendor from invoice where invoice_id = ?)''', (row_id,))
        # company_contact, company_name = cursor.fetchone()
        # cursor.execute("select invoice_id, invoice_amt, invoice_date, invoice_status_id from invoice where invoice_id = ?;", (row_id,))
        # invoice_id, invoice_amt, invoice_date, invoice_status_id = cursor.fetchone()
        # print("invoice status_id is: ", invoice_status_id)
        # body= '''An invoice with Invoice Id '{}' dated {} invoicing {} has been created of the amount being {}.'''.format(row_id, invoice_date, company_name, invoice_amt)
        # sendEmail(company_contact, company_name, invoice_id, invoice_amt, invoice_date, invoice_status_id, body = body)

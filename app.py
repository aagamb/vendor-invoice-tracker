...
def approverApprovedAction():
    ...
    #get invoice
    cur.execute('''
                select company_contact, company_name from company where company_id = (select invoice_vendor from invoice where invoice_id = ?)
            ''', (row_id,))
    company_contact, company_name = cur.fetchone()[0]
    print(company_contact)
    cur.execute("select invoice_id, invoice_amt, invoice_date, invoice_status from invoice where invoice_id = ?;", (row_id,))
    invoice_id, invoice_amt, invoice_date, invoice_status = cur.fetchone()[0]
    print(invoice_status)
    # sendEmail(company_contact, company_name, invoice_id, invoice_amt, invoice_date, invoice_status)
